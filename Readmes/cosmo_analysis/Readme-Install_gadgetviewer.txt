...
gadgetviewer INSTALLATION

DEPENDENCIES: OpenGL/Glut libs; GTK+ (VERSION 2.0) :: En linux Ubuntu 12 se instala con 
...
HDF5, for reading snapshots:
sudo apt-get install libhdf5-serial-dev

LibPNG, for printing out PNG files:
...
Note: If you have installed already another NagBody code, steps 1 and 2 are not necessary. Just check that machine files in the Machine directory have the appropriate compilers and flags.
...
export CC=gcc-mp-6
export CXX=g++-mp-6
export F77=gfortran-mp-6
export FC=gfortran-mp-6
export F90=gfortran-mp-6
...
> gunzip gadgetviewer-1.0.4.tar.gz
> tar xvf gadgetviewer-1.0.4.tar

6. Change to directory:

> cd gadgetviewer-1.0.4

7. Configure, make and install:

./configure --prefix=$HOME/NagBody_pkg/local/gadgetviewer --with-plplot=$HOME/NagBody_pkg/local/plplot 2>&1 | tee configure_gcc.log
make 2>&1 | tee make_gcc.log
make install 2>&1 | tee install_gcc.log

8. Move the directory Ò$HOME/NagBody_pkg/localÓ and make the link:

> mv gadgetviewer/ gadgetviewer-1.0.4_gcc-mp-4.6_gfortran-mp-4.6_nagbody_loal_plplot
> ln -s gadgetviewer-1.0.4_gcc-mp-4.6_gfortran-mp-4.6_nagbody_loal_plplot gadgetviewer
