...
stream nstropen(const_string name, string mode)		
{
    ...
    if (name[0] == '-') {		/* see if '-' or '-num' special file */
        if (streq(mode,"s")) 
            nerror("stropen: no scratch mode allowed in %s",name);
	if (streq(name, "-")) {
	    fds = dup(fileno(inflag ? stdin : stdout));
	    if (fds == -1)
		nerror("stropen: cannot dup %s", inflag ? "stdin" : "stdout");
	} else {
	    fds = atoi(&name[1]);
        }
	res = (stream ) fdopen(fds, streq(mode, "w!") ? "w" : mode);
	if (res == NULL)
	    nerror("stropen: cannot open f.d. %d for %s\n",
		  fds, inflag ? "input" : "output");
        fds=fileno(res);
        if (fds>=NEMO_MAXFD) 
            nerror("stropen: file descriptor too large for ftable. fds=%d",fds);
	    ...
	    if (fds < 0) {
	      if (stat(tempname,&buf)==0)
                nerror("stropen: scratch file \"%s\" already exists", tempname);
	      res = fopen(tempname,"w+");
	    } else
	      res = fdopen(fds,"w+");
            if (res==NULL) 
                nerror("stropen: cannot open scratch file \"%s\"",tempname);
        } else {                    /* "r" or "w" mode */
            if (streq(mode, "w") &&
		!streq(name,".") &&
		stat(tempname, &buf) == 0)
                nerror("stropen: file \"%s\" already exists\n", tempname);
            if (streq(name,".")) {
		...
		canSeek = FALSE;
            ...
            if (res == NULL)
                nerror("stropen: cannot open file \"%s\" for %s\n",
