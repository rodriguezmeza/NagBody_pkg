...
string usage;    /* see program.c or usage.c */
//extern string usage;    /* see program.c or usage.c */
string cvsid;    /* see program.c or cvsid.c */				// PENSAR EN HACER BIEN ESTA CORRECCION!!!!
    ...
    for (i = 1; argv[i] != NULL; i++) {         /* loop over stuff in argv  */
	  ...
	  break;
        ...
        if (posflag) {                          /*   match by position?     */
            ...
            if (i > maxkeys) nerror("Too many un-named arguments");
		...
		streq(argv[i],"-h") || streq(argv[i],"help")) {
	      ...
	      if (keys[i].indexed >= -1) 
		nerror("Cannot match indexed keywords by position");
	      free(keys[i].val);
	      ...
	      keys[i].count++;
            ...
            if (name == NULL)
                nerror("Arg \"%s\" must be named", argv[i]);
            j = findkey(name);                  /*     find index in keys   */
            if (j >= 0) {                       /*     was name found?      */
	      getparam_argc++;			/* count keywords */
	      if (keys[j].count)
		nerror("Parameter \"%s\" duplicated", name);
	      free(keys[j].val);
		...
		kw = kw->next;
		if (kw->indexed == idx+1) 
		  nerror("Duplicated indexed keyword %s",name);
	      }
		...
		nerror("Parameter \"%s\" unknown", name);
...

    ...
    if (help_level & HELP_PROMPT) {
        ...
        if (!isatty(fileno(stdin)) || !isatty(fileno(stdout)))
            nerror("(initparam) Can only INTERACT with terminal");
...
#else   
    nerror("(initparam) INTERACTIVE input not allowed; check help=?");
#endif /* INTERACT */     
    }
...

    ...
    if (help_level&HELP_EDIT) {    /* into editor mode, edit and read keyfile */
        ...
        if (system(keybuf))                         /*  edit the keyword file */
            nerror("(initparam) Error running editor (%s)",keybuf);
...
local string get_macro(char *mname)
{
    ...
    if (n<0)
        nerror("(getparam) macro include file \"%s\" does not exist",mname);
    ...
    fstr = nstropen(mname,"r");      /* open macro file (guaranteed ok)     */
    ...
    while (more) {                   /* loop until no more $ references found */
        ...
        for (i=1; i<nkeys; i++) {                  /* loop over all key=val's */
            ...
            while (cd) {                       /* loop, while more '$' found */
                ...
                if (*c1 == 0)
                    nerror("improper $-reference in %s",keys[i].val);
                ...
                *c3 = 0; /* terminate key */
                ...
                if (j<0) {
                    c4 = getenv(key);
                    if (c4==NULL) 
                        nerror("eval_keys: bad $%s in %s=%s",
                            key,keys[i].key,keys[i].val);
                } else if (i==j) {
                    nerror("eval_keys: recursive %s=%s",keys[i].key,keys[i].val);
                    c4 = "";
                } else
                    c4 = keys[j].val;
...
bool hasvalue(string name)     
{
  ...
  kw = findakey(name);
  if (kw == NULL) nerror("keyword %s does not exist",name);
...
bool updparam(string name)
{
    ...
    if (kw == NULL) nerror("(updparam) \"%s\" unknown keyword",name);
    return kw->upd == 1;
}
...
string getparam(string name)
{
    ...
    kw = findakey(name);
    if (kw == NULL) nerror("(getparam) \"%s\" unknown keyword", name);
    kw->upd = 0;
#if defined(MACROREAD)
    cp = kw->val;
    if (*cp == '@') {
...
string getparam_idx(string name, int idx)
{
    ...
    strcat(key,"#");
    i = findkey(key);
    if (i<0) nerror("(getparam_idx) \"%s\" unknown keyword", name);
    kw = &keys[i];
    if (kw->indexed < -1) nerror("%s is not an indexed keyword",name);
    while (kw->next) {
...
int indexparam(string basename, int idx)
{
  ...
  kw = &keys[i];
  if (kw->indexed < -1) nerror("%s is not an indexed keyword",basename);
  ...
  nerror("indexparam: reached an impossible part of code");
...
int getiparam(string par)
{
    ...
    if (nret < 0)
        nerror("getiparam(%s=%s) parsing error %d, assumed %d\n",
                    par,val,nret,ipar);
...
int getiparam_idx(string par, int idx)
{
    ...
    nret = nemoinpi(val,&ipar,1);
    if (nret < 0)
        nerror("getiparam_idx(%s=%s,%d) parsing error %d, assumed %d\n",
...
long getlparam(string par)
{
    ...
    if (nret < 0)
        nerror("getlparam(%s=%s) parsing error %d assumed %l\n",
                    par,val,nret,lpar);
...
bool getbparam(string par)
{
    ...
    nerror("getbparam: %s=%s not bool", par, val);
    return 0;   /*turboc*/
#else
    nret = nemoinpb(val,&bpar,1);
    if (nret < 0)
        nerror("getbparam(%s=%s) parsing error %d, assumed %d (FALSE)",
                        par,val,nret,bpar);
...
double getdparam(string par)
{
    ...
    if (nret < 0)
        nerror("getdparam(%s=%s) parsing error %d, assumed %g",
                            par,val,nret,dpar);
...
local void setparam (string par, string val, string prompt)
{
    ...
    if (par==NULL || *par==0)
        nerror("setparam: no parameter supplied?");
    if (nkeys==0)
    ...
    if (kw == NULL) {
      ...
      if (i==0)
	nerror("setparam: parameter \"%s\" unknown",par);
	...
	clearerr(stdin);
#if 0
        if (gets(line) == NULL) nerror("Null input");
#else
        // if (fgets(line,80,stdin) == NULL) error("Null input");
	nerror("Can't do prompting anymore until fgets() is fixed");
#endif
        val = line;
    }
    kw->val = scopy(val);
...
#else
    nerror("setparam: not compiled into getparam.c");
...
local int readparam(string buffer,string text)
{
    ...
    if (!isatty(fileno(stdin)))
        nerror("readparam: Cannot use redirected input in interactive mode");
    ...
    *p = 0;                      /* terminate string */
    ...
    return go;
#else
    nerror("readparam: feature not available (missing TIOCSTI)");
#endif
}
#endif /* INTERACT */
...
local int findkey(string name)
{
    ...
    if (count==1) {
        ...
        dprintf(0,"\n");
        nerror("Ambiguous keyword %s=",name);
    }
#endif
...
local keyword *findakey(string name)
{
  ...
  if (*(cp+1)) {        /* ok, this keyword name must be indexed now */
    ...
    *cp = 0;
    ...
    kw = &keys[n];
    if (kw->indexed < -1) 
      nerror("findakey(%s): not an indexed keyword, %s: %d n=%d", 
...
local int addindexed(int i, string keyval, int idx)
{
  ...
  while (kw->next) {        /* link through all indexed ones */
	...
	return 0;
      ...
      nerror("Duplicated indexed keyword %s",keyval);
    }
  }
  kw->next = (keyword *) allocate(sizeof(keyword));
  kw = kw->next;
...
local void writekeys(string mesg)
{
    ...
    if (keyfile==NULL)
        nerror("%s: Cannot write to keyfile \"%s\", - no write permission?",
                                                 mesg, key_filename);
...
local void readkeys(string mesg, bool first)
{
    ...
    if (keyfile==NULL && !first)
        nerror("%s Cannot read keyfile \"%s\" ",mesg,key_filename);
...
#else
    nerror("outkeys= not supported, or recompile with -DOUTKEYS");
#endif
