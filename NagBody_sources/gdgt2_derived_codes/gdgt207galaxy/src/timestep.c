...
void advance_and_find_timesteps(void)
{
  ...
  if(gd.ComovingIntegrationOn)
    {
      ...
      fac3 = pow(gd.Time, 3 * (1 - GAMMA) / 2.0);
  ...
  dt_displacement = gd.MaxSizeTimestep;
#else
...
#ifdef PMGRID
  if(gd.ComovingIntegrationOn)
    dt_gravkickB = get_gravkick_factor(gd.PM_Ti_begstep, gd.Ti_Current) -
      get_gravkick_factor(gd.PM_Ti_begstep, (gd.PM_Ti_begstep + gd.PM_Ti_endstep) / 2);
  else
    dt_gravkickB = (gd.Ti_Current - (gd.PM_Ti_begstep + gd.PM_Ti_endstep) / 2) * gd.Timebase_interval;

  if(gd.PM_Ti_endstep == gd.Ti_Current)	// need to do long-range kick
    {
  ...
  for(i = 0, dispmax = 0, disp2sum = 0; i < NumPart; i++)
      ...
      for(j = 0; j < 3; j++)
	{
      ...
      disp *= 2.0 / (3 * gd.Hubble * gd.Hubble);
  ...
  dmean = pow(P[0].Mass / (gd.Omega0 * 3 * gd.Hubble * gd.Hubble / (8 * M_PI * gd.G)), 1.0 / 3);
  ...
  if(ThisTask == 0)
    {
      printf("\nglass-making:  dmean= %g  global disp-maximum= %g  rms= %g\n\n",
	     dmean, globmax, sqrt(globdisp2sum / gd.TotNumPart));
      fflush(stdout);
    }
  ...
  for(i = 0, dispmax = 0; i < NumPart; i++)
	...
	{
	  P[i].Vel[j] = 0;
	  P[i].Pos[j] += fac * P[i].GravAccel[j] * 2.0 / (3 * gd.Hubble * gd.Hubble);
	  P[i].GravAccel[j] = 0;
	}
    }
#endif
...
#ifdef FLEXSTEPS
  if((gd.Ti_Current % (4 * gd.PresentMinStep)) == 0)
    if(gd.PresentMinStep < TIMEBASE)
      gd.PresentMinStep *= 2;
  ...
  for(i = 0; i < NumPart; i++)
      ...
      if(P[i].Ti_endstep == gd.Ti_Current)
	{
	  ...
	  if(ti_step < gd.PresentMinStep)
	    gd.PresentMinStep = ti_step;
	}
    }

  ti_step = gd.PresentMinStep;
  MPI_Allreduce(&ti_step, &gd.PresentMinStep, 1, MPI_INT, MPI_MIN, MPI_COMM_WORLD);

  if(dt_displacement < gd.MaxSizeTimestep)
    ti_step = (int) (dt_displacement / gd.Timebase_interval);
  else
    ti_step = (int) (gd.MaxSizeTimestep / gd.Timebase_interval);

  // make it a power 2 subdivision
  ti_min = TIMEBASE;
  while(ti_min > ti_step)
    ti_min >>= 1;
  gd.PresentMaxStep = ti_min;


  if(ThisTask == 0)
    printf("Syn Range = %g  PresentMinStep = %d  PresentMaxStep = %d \n",
	   (double) gd.PresentMaxStep / gd.PresentMinStep, gd.PresentMinStep, gd.PresentMaxStep);
  ...
  for(i = 0; i < NumPart; i++)
      ...
      if(P[i].Ti_endstep == gd.Ti_Current)
	{
...
#ifdef FLEXSTEPS
	  ti_grp = P[i].FlexStepGrp % gd.PresentMaxStep;
	  ti_grp = (ti_grp / gd.PresentMinStep) * gd.PresentMinStep;
	  ...
	  if(gd.Ti_Current == TIMEBASE)	// we here finish the last timestep.
	    ti_step = 0;

	  if((TIMEBASE - gd.Ti_Current) < ti_step)	// check that we don't run beyond the end
	    ti_step = TIMEBASE - gd.Ti_Current;
	  ...
	  if(gd.ComovingIntegrationOn)
	    {
	      dt_entr = (tend - tstart) * gd.Timebase_interval;
	      dt_entr2 = (tend - P[i].Ti_endstep) * gd.Timebase_interval;
	    ...
	    {
	      dt_entr = dt_gravkick = dt_hydrokick = (tend - tstart) * gd.Timebase_interval;
	      dt_gravkick2 = dt_hydrokick2 = dt_entr2 = (tend - P[i].Ti_endstep) * gd.Timebase_interval;
	    }
	  ...
	  if(P[i].Type == 0)	// SPH stuff
		...
		SphP[i].Entropy *= 0.5;
	      ...
	      if(gd.MinEgySpec)
		{
		  minentropy = gd.MinEgySpec * GAMMA_MINUS1 / pow(SphP[i].Density * a3inv, GAMMA_MINUS1);
	      ...
	      dt_entr = ti_step / 2 * gd.Timebase_interval;
...
//
	  if(gd.NumForcesSinceLastDomainDecomp < gd.TotNumPart * gd.TreeDomainUpdateFrequency)
	    {
	      no = Father[i];
	      while(no >= 0)
		{
  ...
  if(gd.PM_Ti_endstep == gd.Ti_Current)	// need to do long-range kick
    {
      ti_step = TIMEBASE;
      while(ti_step > (dt_displacement / gd.Timebase_interval))
	ti_step >>= 1;

      if(ti_step > (gd.PM_Ti_endstep - gd.PM_Ti_begstep))	// PM-timestep wants to increase
	{
	  // we only increase if an integer number of steps will bring us to the end
	  if(((TIMEBASE - gd.PM_Ti_endstep) % ti_step) > 0)
	    ti_step = gd.PM_Ti_endstep - gd.PM_Ti_begstep;	// leave at old step
	}

      if(gd.Ti_Current == TIMEBASE)	// we here finish the last timestep.
	ti_step = 0;

      tstart = (gd.PM_Ti_begstep + gd.PM_Ti_endstep) / 2;
      tend = gd.PM_Ti_endstep + ti_step / 2;

      if(gd.ComovingIntegrationOn)
	dt_gravkick = get_gravkick_factor(tstart, tend);
      else
	dt_gravkick = (tend - tstart) * gd.Timebase_interval;

      gd.PM_Ti_begstep = gd.PM_Ti_endstep;
      gd.PM_Ti_endstep = gd.PM_Ti_begstep + ti_step;

      if(gd.ComovingIntegrationOn)
	dt_gravkickB = -get_gravkick_factor(gd.PM_Ti_begstep, (gd.PM_Ti_begstep + gd.PM_Ti_endstep) / 2);
      else
	dt_gravkickB =
	  -((gd.PM_Ti_begstep + gd.PM_Ti_endstep) / 2 - gd.PM_Ti_begstep) * gd.Timebase_interval;
	  ...
	  if(P[i].Type == 0)
	    {
	      if(gd.ComovingIntegrationOn)
		{
		  dt_gravkickA = get_gravkick_factor(P[i].Ti_begstep, gd.Ti_Current) -
		    get_gravkick_factor(P[i].Ti_begstep, (P[i].Ti_begstep + P[i].Ti_endstep) / 2);
		  dt_hydrokick = get_hydrokick_factor(P[i].Ti_begstep, gd.Ti_Current) -
		    get_hydrokick_factor(P[i].Ti_begstep, (P[i].Ti_begstep + P[i].Ti_endstep) / 2);
		}
	      else
		dt_gravkickA = dt_hydrokick =
		  (gd.Ti_Current - (P[i].Ti_begstep + P[i].Ti_endstep) / 2) * gd.Timebase_interval;
  ...
  t1 = second();
  gd.CPU_TimeLine += timediff(t0, t1);
}
...
)
{
  ...
  switch (gd.TypeOfTimestepCriterion)
    {
    case 0:
      if(flag > 0)
	{
	  dt = flag * gd.Timebase_interval;
	  dt /= hubble_a;	// convert dloga to physical timestep
	  ac = 2 * gd.ErrTolIntAccuracy * atime * gd.SofteningTable[P[p].Type] / (dt * dt);
	  *aphys = ac;
	  return flag;
	}
      dt = dt_accel = sqrt(2 * gd.ErrTolIntAccuracy * atime * gd.SofteningTable[P[p].Type] / ac);
#ifdef ADAPTIVE_GRAVSOFT_FORGAS
      if(P[p].Type == 0)
	dt = dt_accel = sqrt(2 * gd.ErrTolIntAccuracy * atime * SphP[p].Hsml / 2.8 / ac);
#endif
      break;
    default:
      endrun_mpi(0,888);
  ...
  if(P[p].Type == 0)
    {
      ...
      if(gd.ComovingIntegrationOn)
	dt_courant = 2 * gd.CourantFac * gd.Time * SphP[p].Hsml / (fac3 * SphP[p].MaxSignalVel);
      else
	dt_courant = 2 * gd.CourantFac * SphP[p].Hsml / SphP[p].MaxSignalVel;
...
//     hubble_a=1.
  ...
  if(dt >= gd.MaxSizeTimestep)
    dt = gd.MaxSizeTimestep;

  if(dt >= dt_displacement)
    dt = dt_displacement;

  if(dt < gd.MinSizeTimestep)
    {
      ...
      if(P[p].Type == 0)
	{
	  printf
	    ("Part-ID=%d  dt=%g dtc=%g ac=%g xyz=(%g|%g|%g)  hsml=%g  maxsignalvel=%g dt0=%g eps=%g\n",
	     ...
	     sqrt(2 * gd.ErrTolIntAccuracy * atime * gd.SofteningTable[P[p].Type] / ac) * hubble_a,
	     gd.SofteningTable[P[p].Type]);
	}
      else
	{
      ...
      endrun_mpi(0,888);
#endif
      dt = gd.MinSizeTimestep;
    }

  ti_step = dt / gd.Timebase_interval;
  ...
  if(!(ti_step > 0 && ti_step < TIMEBASE))
    {
      printf("\nError: A timestep of size zero was assigned on the integer timeline!\n"
	     ...
	     ThisTask, (int) P[p].ID, dt, gd.Timebase_interval, ti_step, ac,
	  ...
	  if(count_sum[type] > 0)
	    {
	      if(type == 0)
		dmean =
		  pow(min_mass[type] / (gd.OmegaBaryon * 3 * gd.Hubble * gd.Hubble / (8 * M_PI * gd.G)),
		      1.0 / 3);
	      else
		dmean =
		  pow(min_mass[type] /
		      ((gd.Omega0 - gd.OmegaBaryon) * 3 * gd.Hubble * gd.Hubble / (8 * M_PI * gd.G)),
		      1.0 / 3);

	      dt = gd.MaxRMSDisplacementFac * hfac * dmean / sqrt(v_sum[type] / count_sum[type]);
...
#endif
	      if(asmth < dmean)
		dt = gd.MaxRMSDisplacementFac * hfac * asmth / sqrt(v_sum[type] / count_sum[type]);
#endif

	      if(ThisTask == 0)
		printf("type=%d  dmean=%g asmth=%g minmass=%g a=%g  sqrt(<p^2>)=%g  dlogmax=%g\n",
		       type, dmean, asmth, min_mass[type], gd.Time, sqrt(v_sum[type] / count_sum[type]), dt);
