...
//        restart(RestartFlag);
      ...
      gd.MinSizeTimestep = all.MinSizeTimestep;
      gd.MaxSizeTimestep = all.MaxSizeTimestep;
      gd.BufferSize = all.BufferSize;
      gd.BunchSizeForce = all.BunchSizeForce;
      gd.BunchSizeDensity = all.BunchSizeDensity;
      gd.BunchSizeHydro = all.BunchSizeHydro;
      gd.BunchSizeDomain = all.BunchSizeDomain;

      gd.TimeLimitCPU = all.TimeLimitCPU;
      gd.ResubmitOn = all.ResubmitOn;
      gd.TimeBetSnapshot = all.TimeBetSnapshot;
      gd.TimeBetStatistics = all.TimeBetStatistics;
      gd.CpuTimeBetRestartFile = all.CpuTimeBetRestartFile;
      gd.ErrTolIntAccuracy = all.ErrTolIntAccuracy;
      gd.MaxRMSDisplacementFac = all.MaxRMSDisplacementFac;

      gd.ErrTolForceAcc = all.ErrTolForceAcc;

      gd.TypeOfTimestepCriterion = all.TypeOfTimestepCriterion;
      gd.TypeOfOpeningCriterion = all.TypeOfOpeningCriterion;
      gd.NumFilesWrittenInParallel = all.NumFilesWrittenInParallel;
      gd.TreeDomainUpdateFrequency = all.TreeDomainUpdateFrequency;

      gd.SnapFormat = all.SnapFormat;
      gd.NumFilesPerSnapshot = all.NumFilesPerSnapshot;
      gd.MaxNumNgbDeviation = all.MaxNumNgbDeviation;
      gd.ArtBulkViscConst = all.ArtBulkViscConst;


      gd.OutputListOn = all.OutputListOn;
      gd.CourantFac = all.CourantFac;

      gd.OutputListLength = all.OutputListLength;
      memcpy(gd.OutputListTimes, all.OutputListTimes, sizeof(double) * gd.OutputListLength);


      strcpy(gd.ResubmitCommand, all.ResubmitCommand);
      strcpy(gd.OutputListFilename, all.OutputListFilename);
      strcpy(gd.OutputDir, all.OutputDir);
      strcpy(gd.RestartFile, all.RestartFile);
      strcpy(gd.EnergyFile, all.EnergyFile);
      strcpy(gd.InfoFile, all.InfoFile);
      strcpy(gd.CpuFile, all.CpuFile);
      strcpy(gd.TimingsFile, all.TimingsFile);
      strcpy(gd.SnapshotFileBase, all.SnapshotFileBase);

      if(gd.TimeMax != all.TimeMax)
	readjust_timebase(gd.TimeMax, all.TimeMax);
    }
...
//  if(RestartFlag == 0)
  ...
  if(!(FdInfo = fopen(buf, mode)))
    ...
    sprintf(buf, "%s%s2", gd.OutputDir, gd.EnergyFile);
    if(!(FdEnergy2 = fopen(buf, mode)))
    {
        printf("error in opening file '%s'\n", buf);
        endrun_mpi(0,1);
    }
