 ...
 *  [0,gd.BoxSize] is only done before the domain decomposition, or for
...
void move_particles(int time0, int time1)
{
  ...
  if(gd.ComovingIntegrationOn)
    ...
    {
      dt_drift = dt_gravkick = dt_hydrokick = (time1 - time0) * gd.Timebase_interval;
    }
  ...
  for(i = 0; i < NumPart; i++)
      ...
      if(P[i].Type == 0)
	{
	  ...
	  dt_entr = (time1 - (P[i].Ti_begstep + P[i].Ti_endstep) / 2) * gd.Timebase_interval;
  ...
  if(gd.NumForcesSinceLastDomainDecomp < gd.TotNumPart * gd.TreeDomainUpdateFrequency)
	...
	for(j = 0; j < 3; j++)
	  Nodes[gd.MaxPart + i].u.d.s[j] += Extnodes[gd.MaxPart + i].vs[j] * dt_drift;
