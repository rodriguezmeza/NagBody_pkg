...
/*#ifdef PERIODIC
      ...
      if(ThisTask == 0)
	{
      ...
      Key = malloc(sizeof(peanokey) * gd.MaxPart);
      KeySorted = malloc(sizeof(peanokey) * gd.MaxPart);
      ...
      maxload = gd.MaxPart * REDUC_FAC;
      maxloadsph = gd.MaxPartSph * REDUC_FAC;
      ...
      if(ThisTask == 0)
	{
      ...
      t1 = second();
      gd.CPU_Domain += timediff(t0, t1);

#ifdef PEANOHILBERT
      t0 = second();
      peano_hilbert_order();
      t1 = second();
      gd.CPU_Peano += timediff(t0, t1);
#endif
...
void domain_decompose(void)
{
  ...
  for(ngrp = i + 1; ngrp < 6; ngrp++)
    {
      if(Ntype[ngrp] > 0)
	if(gd.SofteningTable[ngrp] != gd.SofteningTable[i])
	  {
	    if(ThisTask == 0)
	      {
...
void domain_findExchangeNumbers(int task, int partner, int sphflag, int *send, int *recv)
{
  ...
  maxsendA = imin(ntobesentA, gd.BunchSizeDomain);
  maxsendB = imin(ntobesentB, gd.BunchSizeDomain);
  ...
  do
    {
      ...
      maxsendA = imin(gd.MaxPart - numpartB + maxsendB, maxsendA);
      maxsendB = imin(gd.MaxPart - numpartA + maxsendA, maxsendB);
    }
  ...
  if(sphflag == 1)
      ...
      do
	{
	  ...
	  maxsendA = imin(gd.MaxPartSph - numpartsphB + maxsendB, maxsendA);
	  maxsendB = imin(gd.MaxPartSph - numpartsphA + maxsendA, maxsendB);
	}
      while((maxsendA != maxsendA_old) || (maxsendB != maxsendB_old));
    }
...
void domain_exchangeParticles(int partner, int sphflag, int send_count, int recv_count)
{
  ...
  for(rep = 0; rep < 2; rep++)
      ...
      if((rep == 0 && ThisTask < partner) || (rep == 1 && ThisTask > partner))
	{
	  if(send_count > 0)
	    {
	      MPI_Ssend(&DomainPartBuf[0], send_count * sizeof(particle_data), MPI_BYTE, partner,
	      ...
	      if(sphflag)
		MPI_Ssend(&DomainSphBuf[0], send_count * sizeof(sph_particle_data), MPI_BYTE, partner,
      ...
      if((rep == 1 && ThisTask < partner) || (rep == 0 && ThisTask > partner))
	{
	  if(recv_count > 0)
	      ...
	      if(sphflag)
		{
		  ...
		  MPI_Recv(&P[N_gas], recv_count * sizeof(particle_data), MPI_BYTE, partner, TAG_PDATA,
			   MPI_COMM_WORLD, &status);
		  MPI_Recv(&Key[N_gas], recv_count * sizeof(peanokey), MPI_BYTE, partner, TAG_KEY,
			   MPI_COMM_WORLD, &status);
		  MPI_Recv(&SphP[N_gas], recv_count * sizeof(sph_particle_data), MPI_BYTE, partner,
		...
		{
		  MPI_Recv(&P[NumPart], recv_count * sizeof(particle_data), MPI_BYTE, partner,
...
void domain_determineTopTree(void)
{
  ...
  TopNodes[0].Size = PEANOCELLS;
  TopNodes[0].StartKey = 0;
  TopNodes[0].Count = gd.TotNumPart;
  TopNodes[0].Pstart = 0;
...
void domain_topsplit_local(int node, peanokey startkey)
{
  ...
  if(TopNodes[node].Size >= 8)
	...
	{
	  sub = TopNodes[node].Daughter + i;
	  if(TopNodes[sub].Count > gd.TotNumPart / (TOPNODEFACTOR * NTask * NTask))
...
void domain_topsplit(int node, peanokey startkey)
{
  ...
  if(TopNodes[node].Size >= 8)
	...
	{
	  sub = TopNodes[node].Daughter + i;
	  if(TopNodes[sub].Count > gd.TotNumPart / (TOPNODEFACTOR * NTask))
