...
void compute_global_quantities_of_system(void)
{
  ...
  for(i = 0; i < NumPart; i++)
      ...
      if(gd.ComovingIntegrationOn)
	{
	  ...
	  dt_gravkick = get_gravkick_factor(P[i].Ti_begstep, gd.Ti_Current) -
...
#ifdef PMGRID
      if(gd.ComovingIntegrationOn)
	dt_gravkick = get_gravkick_factor(gd.PM_Ti_begstep, gd.Ti_Current) -
	  get_gravkick_factor(gd.PM_Ti_begstep, (gd.PM_Ti_begstep + gd.PM_Ti_endstep) / 2);
      else
	dt_gravkick = (gd.Ti_Current - (gd.PM_Ti_begstep + gd.PM_Ti_endstep) / 2) * gd.Timebase_interval;
  ...
  MPI_Bcast(&SysState, sizeof(state_of_system), MPI_BYTE, 0, MPI_COMM_WORLD);
}
