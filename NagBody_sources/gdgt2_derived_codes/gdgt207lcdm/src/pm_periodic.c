...
void pm_init_periodic(void)
{
  ...
  gd.Asmth[0] = ASMTH * gd.BoxSize / PMGRID;
...
void pmforce_periodic(void)
{
  ...
  asmth2 = (2 * M_PI) * gd.Asmth[0] / gd.BoxSize;
  asmth2 *= asmth2;

  fac = gd.G / (M_PI * gd.BoxSize);	// to get potential
  fac *= 1 / (2 * gd.BoxSize / PMGRID);	// for finite differencing
