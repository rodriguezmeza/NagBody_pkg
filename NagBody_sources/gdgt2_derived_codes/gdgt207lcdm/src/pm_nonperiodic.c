...
void pm_init_regionsize(void)
{
  ...
  MPI_Allreduce(xmin, gd.Xmintot, 6, MPI_DOUBLE, MPI_MIN, MPI_COMM_WORLD);
  MPI_Allreduce(xmax, gd.Xmaxtot, 6, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);

  for(j = 0; j < 2; j++)
    {
      gd.TotalMeshSize[j] = gd.Xmaxtot[j][0] - gd.Xmintot[j][0];
      gd.TotalMeshSize[j] = dmax(gd.TotalMeshSize[j], gd.Xmaxtot[j][1] - gd.Xmintot[j][1]);
      gd.TotalMeshSize[j] = dmax(gd.TotalMeshSize[j], gd.Xmaxtot[j][2] - gd.Xmintot[j][2]);
#ifdef ENLARGEREGION
      gd.TotalMeshSize[j] *= ENLARGEREGION;
#endif

      /* symmetrize the box onto the center */
      for(i = 0; i < 3; i++)
	{
	  gd.Xmintot[j][i] = (gd.Xmintot[j][i] + gd.Xmaxtot[j][i]) / 2 - gd.TotalMeshSize[j] / 2;
	  gd.Xmaxtot[j][i] = gd.Xmintot[j][i] + gd.TotalMeshSize[j];
	}
    }
  ...
  for(j = 0; j < 2; j++)
    {
      meshinner[j] = gd.TotalMeshSize[j];
      gd.TotalMeshSize[j] *= 2.001 * (GRID) / ((double) (GRID - 2 - 8));
    }
  ...
  for(j = 0; j < 2; j++)
    for(i = 0; i < 3; i++)
      {
	gd.Corner[j][i] = gd.Xmintot[j][i] - 2.0005 * gd.TotalMeshSize[j] / GRID;
	gd.UpperCorner[j][i] = gd.Corner[j][i] + (GRID / 2 - 1) * (gd.TotalMeshSize[j] / GRID);
      }


#ifndef PERIODIC
  gd.Asmth[0] = ASMTH * gd.TotalMeshSize[0] / GRID;
  gd.Rcut[0] = RCUT * gd.Asmth[0];
#endif

#ifdef PLACEHIGHRESREGION
  gd.Asmth[1] = ASMTH * gd.TotalMeshSize[1] / GRID;
  gd.Rcut[1] = RCUT * gd.Asmth[1];
#endif

#ifdef PLACEHIGHRESREGION
  if(2 * gd.TotalMeshSize[1] / GRID < gd.Rcut[0])
    {
      gd.TotalMeshSize[1] = 2 * (meshinner[1] + 2 * gd.Rcut[0]) * (GRID) / ((double) (GRID - 2));

      for(i = 0; i < 3; i++)
	{
	  gd.Corner[1][i] = gd.Xmintot[1][i] - 1.0001 * gd.Rcut[0];
	  gd.UpperCorner[1][i] = gd.Corner[1][i] + (GRID / 2 - 1) * (gd.TotalMeshSize[1] / GRID);
	}

      if(2 * gd.TotalMeshSize[1] / GRID > gd.Rcut[0])
	{
	  gd.TotalMeshSize[1] = 2 * (meshinner[1] + 2 * gd.Rcut[0]) * (GRID) / ((double) (GRID - 10));

	  for(i = 0; i < 3; i++)
	    {
	      gd.Corner[1][i] = gd.Xmintot[1][i] - 1.0001 * (gd.Rcut[0] + 2 * gd.TotalMeshSize[1] / GRID);
	      gd.UpperCorner[1][i] = gd.Corner[1][i] + (GRID / 2 - 1) * (gd.TotalMeshSize[1] / GRID);
	    }
	}
  ...
  if(ThisTask == 0)
    {
      ...
      printf("(%g|%g|%g)  -> (%g|%g|%g)   ext=%g  totmeshsize=%g  meshsize=%g\n\n",
	     gd.Xmintot[0][0], gd.Xmintot[0][1], gd.Xmintot[0][2],
	     gd.Xmaxtot[0][0], gd.Xmaxtot[0][1], gd.Xmaxtot[0][2], meshinner[0], gd.TotalMeshSize[0],
	     gd.TotalMeshSize[0] / GRID);
#endif
#ifdef PLACEHIGHRESREGION
      printf("\nAllowed region for isolated PM mesh (high-res):\n");
      printf("(%g|%g|%g)  -> (%g|%g|%g)   ext=%g  totmeshsize=%g  meshsize=%g\n\n",
	     gd.Xmintot[1][0], gd.Xmintot[1][1], gd.Xmintot[1][2],
	     gd.Xmaxtot[1][0], gd.Xmaxtot[1][1], gd.Xmaxtot[1][2],
	     meshinner[1], gd.TotalMeshSize[1], gd.TotalMeshSize[1] / GRID);
#endif
    }
...
void pm_setup_nonperiodic_kernel(void)
{
  ...
  for(i = slabstart_x; i < (slabstart_x + nslab_x); i++)
      ...
      for(k = 0; k < GRID; k++)
	{
	  ...
	  fac = erfc(u * gd.Asmth[1] / gd.Asmth[0]) - erfc(u);

	  if(r > 0)
	    kernel[1][GRID * GRID2 * (i - slabstart_x) + GRID2 * j + k] = -fac / r;
	  else
	    {
	      fac = 1 - gd.Asmth[1] / gd.Asmth[0];
...
int pmforce_nonperiodic(int grnr)
{
  ...
  fac = gd.G / pow(gd.TotalMeshSize[grnr], 4) * pow(gd.TotalMeshSize[grnr] / GRID, 3);	/* to get potential */
  fac *= 1 / (2 * gd.TotalMeshSize[grnr] / GRID);	/* for finite differencing */

  to_slab_fac = GRID / gd.TotalMeshSize[grnr];
  ...
  for(i = 0, flag = 0; i < NumPart; i++)
...
#endif
	{
	  for(j = 0; j < 3; j++)
	    {
	      if(P[i].Pos[j] < gd.Xmintot[grnr][j] || P[i].Pos[j] > gd.Xmaxtot[grnr][j])
		{
		  if(flag == 0)
		    {
		      printf
	...
	continue;
      ...
      if(P[i].Pos[0] >= gd.Corner[grnr][0] && P[i].Pos[0] < gd.UpperCorner[grnr][0])
	if(P[i].Pos[1] >= gd.Corner[grnr][1] && P[i].Pos[1] < gd.UpperCorner[grnr][1])
	  if(P[i].Pos[2] >= gd.Corner[grnr][2] && P[i].Pos[2] < gd.UpperCorner[grnr][2])
	    {
	      for(j = 0; j < 3; j++)
		{
		  slab = to_slab_fac * (P[i].Pos[j] - gd.Corner[grnr][j]);
  ...
  for(i = 0; i < NumPart; i++)
	...
	continue;
      ...
      slab_x = to_slab_fac * (P[i].Pos[0] - gd.Corner[grnr][0]);
      dx = to_slab_fac * (P[i].Pos[0] - gd.Corner[grnr][0]) - slab_x;
      slab_x -= meshmin[0];
      slab_xx = slab_x + 1;

      slab_y = to_slab_fac * (P[i].Pos[1] - gd.Corner[grnr][1]);
      dy = to_slab_fac * (P[i].Pos[1] - gd.Corner[grnr][1]) - slab_y;
      slab_y -= meshmin[1];
      slab_yy = slab_y + 1;

      slab_z = to_slab_fac * (P[i].Pos[2] - gd.Corner[grnr][2]);
      dz = to_slab_fac * (P[i].Pos[2] - gd.Corner[grnr][2]) - slab_z;
      slab_z -= meshmin[2];
      slab_zz = slab_z + 1;
