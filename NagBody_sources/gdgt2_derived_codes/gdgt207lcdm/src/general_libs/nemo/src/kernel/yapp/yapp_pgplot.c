 ...
 *	11-aug-98       keep the first colors fixed to fore/back-ground   PJT
...
void plpalette(real *r, real *g, real *b, int nc)
{
    ...
    if (nc > MAXCOLOR)
	nerror("plpalette: cannot define more than %d colors", MAXCOLOR);
    ncolors = nc;
    for (i = 0; i < ncolors; i++) {
	red[i] = r[i];
	green[i] = g[i];
...
void pllut(string fname, bool compress)
{
    ...
    cstr = nstropen(fname, "r");
    ncolors = 0;
    while (fgets(line,256,cstr)) {
       if(ncolors>=MAXCOLOR) nerror("(%d/%d): Too many colors in %s",
