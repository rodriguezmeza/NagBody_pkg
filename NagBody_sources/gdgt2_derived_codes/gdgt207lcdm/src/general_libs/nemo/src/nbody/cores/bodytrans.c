...
#include <ctype.h>
#include <unistd.h>
#include "../../../include/mathlinker.h"
#include "../../../include/bodytransc.h"
     ...
     /* string fname;                   optional filename for object file */
{
	...
	cflags = getenv("CFLAGS");
        ...
        if (system(cmmd) != 0) {
            ...
            system(cmmd);
#endif
            nerror("bodytrans(): could not compile");
	}
#if defined(LOADOBJ3)
	...
	dprintf(2,"bodytrans: %s\n",cmmd);
	if (system(cmmd) != 0)
		nerror("bodytrans: could not move link files");
	    ...
	    dprintf(2,"cmd: %s\n",cmmd);
            if (system(cmmd) != 0) {
                sprintf(cmmd,"rm -f %s",edbbak);    /* end file locking */
                system(cmmd);
                nerror("bodytrans(): could not copy");
            } else
    ...
    result = findfn(func);
    if (result == NULL) {
      nerror("Cant find %s (from findfn)", func);
...
void bodytrans_dummy_for_c(void)
{
    ...
    nerror("bodytrans.c: Cannot call dummy_for_c - included to fool linkers");
    (void) spline();
    (void) bessi0();
    (void) bessk0();
    (void) bessi1();
...
string cvsid="$Id: bodytrans.c,v 1.16 2006/08/02 15:58:05 pteuben Exp $";
