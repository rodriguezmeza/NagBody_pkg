...
// SIRD
local int FindingBodyNearestToBoxCenter(void);     // CHECK 2D --- OK!!!
local void testdata_sp_07(void);            // FCC structure default (SIRD)
...
// SIRD:
#define FCC_IC_SIRD        7                // FCC structure default (SIRD)
...
// SIRD:
        case FCC_IC_SIRD:                   // FCC structure default (SIRD)
            testdata_sp_07(); break;
        case NULLMETHOD:
            printf("\n\tdefault IC ...\n");
...
local void testdata_sp_07(void)             // FCC structure default (SIRD)
{
    ...
    fprintf(gd.outlog,
        "\ntestdata_sp_07: Simple parallelepiped box : smaller cubic boxes (FCC_SIRD)\n");

    strcpy(gd.model_comment, "Simple Parallelepiped Box Model [7] (FCC_SIRD) created");
                ...
                for (j = 0; j < 2; j ++) {
                    ...
                    if (i1<=gd.nbc1) {
                        ...
                        if (i2<=gd.nbc2) {
                            ...
                            Id(p) = p-bodytab+1;
//                            Type(p) = BODY2;
                            Type(p) = BODY1;        // SIRD: All bodies are S
                            Mass(p) = gd.mass2;
                            tmass += Mass(p);
                            ++p; ++i2;  ++i2tot;
                        }
...
#define MFRAC  0.999
    ...
    DO_BODY(p, bodytab, bodytab+gd.nbody) {
        tofD(p) = 1.0E20;
        tofR(p) = 1.0E20;
        tofInfectionElapsed(p) = 0.;
    }
    infectedBody=FindingBodyNearestToBoxCenter();     // CHECK 2D --- OK!!!
    Type(bodytab+infectedBody-1) = BODY2;
    x = xrandom(0.0, MFRAC);
    if (x==0.) x=1.0E-20;
    tofD(bodytab+infectedBody-1) = cmd.infectedRelaxtionTimeD*rlog(1/x);
    x = xrandom(0.0, MFRAC);
    if (x==0.) x=1.0E-20;
    tofR(bodytab+infectedBody-1) = cmd.infectedRelaxtionTimeR*rlog(1/x);
    tofInfectionElapsed(bodytab+infectedBody-1) = 0.;
    gd.SuscSum=gd.nbody-1;
    gd.InfSum=1;
    gd.InfSumAcum=1;
    gd.RelSum=0;
    gd.DeathSum=0;
    fprintf(stdout,"\nSIRD model ICs: %g %g %g %g\n",
            gd.SuscSum, gd.InfSum, gd.RelSum, gd.DeathSum);
#undef MFRAC

    fprintf(stdout,"\nFCC-IC_SIRD - cpu time : %g\n\n", cputime() - cpustart);
} // End icModel 7 (FFC_SIRD)
...
local int FindingBodyNearestToBoxCenter(void)     // CHECK 2D --- OK!!!
{
    ...
    fprintf(gd.outlog,"\nFinding body nearest to the box center ... ");
    ...
    distMin = lmax;
    qmin = bodytab;
    DO_BODY(q, bodytab, bodytab+gd.nbody) {
        DOTPSUBV(drpq2, dr, cmpos, Pos(q));
        DO_COORD(k)
            dr[k]=dr[k]-((real)(nint(dr[k]/gdforce.Box[k])))*gdforce.Box[k];
        DOTVP(drpq2, dr, dr);
        drpq = rsqrt(drpq2);
    ...
    cBody = qmin - bodytab + 1;
    fprintf(gd.outlog,"Id of body at the box center=%d\n",cBody);
    fflush(gd.outlog);
    return cBody;
}
