...
// Considerar otra manera de dar la misma informacion ...
...
// Considerar otra forma de dar la misma informacion
...
// BORRAR: solo es un test:
// SI AL HACER ADJUSTSIRD CAMBIAN VELOCIDADES EKIN and CM DEBEn AJUSTARSE:
//if (cmd.adjustTemperature)
//    AdjustTemp(Ekin);
//else
//    if (gd.nstep < cmd.stepEquil)
...
// SIRD:
local void AdjustSIRD(void)                        // CHECK 2D --- OK!!!
{
    real mtot;
    bodyptr p, q;
    int k;
...
printf("\nAdjusting SIRD body tags ...\n");

    printf("\nChecking infected body neigbourhood ...\n");
    DO_BODY(p, bodytab, bodytab+gd.nbody) {
// BODY2:
        if (Type(p)==BODY2) {
            DO_BODY(q, bodytab, bodytab+gd.nbody) {
                if (q != p && Type(q) == BODY1) {
                    ...
                    DOTVP(drpq2, dr, dr);
                    drpq = rsqrt(drpq2);
                    if (drpq < cmd.infectionMinDist) {
                        x = xrandom(0.0, MFRAC);
                        if (x < cmd.infectionProb) {
                            Type(q)=BODY2;
                            x = xrandom(0.0, MFRAC);
                            if (x==0.) x=1.0E-20;
                            tofD(q) = cmd.infectedRelaxtionTimeD*rlog(1/x);
                            x = xrandom(0.0, MFRAC);
                            if (x==0.) x=1.0E-20;
                            tofR(q) = cmd.infectedRelaxtionTimeR*rlog(1/x);
                            tofInfectionElapsed(q) = 0.;
                            ++gd.InfSum;
                            ++gd.InfSumAcum;
                            --gd.SuscSum;
                            fprintf(stdout,"Found infected %d\n",q-bodytab+1);
                        }
                    }
                }
            }
            tofInfectionElapsed(p) += gd.dtime;
            if (tofInfectionElapsed(p) > tofD(p)) {
                Type(p)=BODY4;
                CLRV(Vel(p));
                ++gd.DeathSum;
                --gd.InfSum;
                ...
                if (tofInfectionElapsed(p) > tofR(p)) {
                    Type(p)=BODY3;
                    ++gd.RelSum;
                    --gd.InfSum;
                    tofInfectionElapsed(p) = 0.;
