...
int nemorinpi(string var, int *xvar,  int n, int defvar, bool repeated)
{
  ...
  if (var == 0 || *var == 0) {
    ...
    if (nret <= 0) nerror("nemorinpi: parsing error %d in %s",nret,var);
    for (i=nret; i<n; i++)
...
int nemorinpd(string var,  double *xvar, int n,double defvar, bool repeated)
{
  ...
  if (var == 0 || *var == 0) {
    ...
    if (nret <= 0) nerror("nemorinpd: parsing error %d in %s",nret,var);
    for (i=nret; i<n; i++)
      xvar[i] = repeated ? xvar[i-1] : defvar;
    return nret;
  }
...
int nemorinpf(string var, float *xvar,  int n, float defvar, bool repeated)
{
  ...
  if (var == 0 || *var == 0) {
    ...
    if (nret <= 0) nerror("nemorinpf: parsing error %d in %s",nret,var);
    for (i=nret; i<n; i++)
      xvar[i] = repeated ? xvar[i-1] : defvar;
    return nret;
  }
...
int nemorinpl(string var, long *xvar,  int n, long defvar, bool repeated)
{
  ...
  if (var == 0 || *var == 0) {
    ...
    if (nret <= 0) nerror("nemorinpl: parsing error %d in %s",nret,var);
    for (i=nret; i<n; i++)
      xvar[i] = repeated ? xvar[i-1] : defvar;
    return nret;
  }
...
int nemorinpb(string var, bool *xvar,  int n, bool defvar, bool repeated)
{
  ...
  if (var == 0 || *var == 0) {
    ...
    if (nret <= 0) nerror("nemorinpb: parsing error %d in %s",nret,var);
    for (i=nret; i<n; i++)
      xvar[i] = repeated ? xvar[i-1] : defvar;
    return nret;
  }
...
string cvsid="$Id: nemoinp.c,v 1.13 2006/02/05 05:05:47 pteuben Exp $";
...
void nemo_main(void)
{
    ...
    if (Qdms) {
      ...
      if (nret < 0) nerror("Parsing dms expression");
      for (i=0; i<nret; i++)
	printf("%g\n",dms[i]);
      return;  /* for now */
    }
    ...
    if (Qint) {
        ...
        if (nret == -23)
            nerror("Too many items in list, use a bigger nmax=%d",nx);
        else if (nret < 0)
            nerror("nemoinp (%s) parsing error (%d)",cp,nret);
