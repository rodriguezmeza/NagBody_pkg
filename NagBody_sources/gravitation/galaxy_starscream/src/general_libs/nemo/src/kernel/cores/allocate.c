...
void *allocate(int nb)
{
    ...
    if (nb < 0) nerror("allocate < 0: cannot allocate %d bytes",nb);
	...
	nerror("allocate: not enough memory for %d bytes", nb);
    }
...
void *reallocate(void *bp, int nb)
{
    ...
    /* how should this kind of error be processed ? */
    if (nb < 0) nerror("reallocate: cannot allocate %d bytes",nb);
    if (nb == 0) nb++;
    if(bp==NULL)
        mem = (void *) calloc((size_t)nb, 1);
    else
    ...
    if (mem == NULL)  {
	nerror("reallocate: not enough memory for %d bytes", nb);
    }
