...
//
// MAR: Hemos modificado la estructura va_list ... ahora es va_list_nemo ...
// ya que aparece en un encabezado de /usr/include ...
//

#include "../../../include/stdinc.h"
#include <unistd.h>
#include "../../../include/strlib.h"
#include "../../../include/filestruct.h"
#include "../../../include/extstring.h"
#include "filesecret.h"
#include "../../tools/unproto/stdarg.h"
...
void copy_item(stream ostr, stream istr, string tag)
{
	...
	nerror("copy_item: tag %s not found", tag);
	...
	if(dlen<0) nerror("copy_item: %s with dlen=%d",tag,dlen);    /* yuck */
	buf = (byte *) calloc(dlen,1);		/*   get space for a buffer */
	if (buf == NULL)			/*   and check for error    */
	    nerror("copy_item: item %s: not enuf memory", tag);
...
void copy_item_cvt(stream ostr, stream istr, string tag, string *cvt)
{
    ...
    if (! get_tag_ok(istr, tag))		/* prevent obvious errors   */
	nerror("copy_item_cvt: tag %s not found", tag);
    type = get_type(istr, tag);			/* find out type of data    */
    cvtlen = xstrlen(cvt,sizeof(string)) - 1;   /* # conversions            */
    if (! streq(type, SetType)) {		/* a basic type or array?   */
	dims = get_dims(istr, tag);		/*   find out about shape   */
	dlen = get_dlen(istr, tag);		/*   and length in bytes    */
	if(dlen<0) nerror("copy_item_cvt: %s with dlen=%d",tag,dlen); 
	bufin = (byte *) calloc(dlen,1);	/*   get space for a buffer */
	if (bufin == NULL)			/*   and check for error    */
	    nerror("copy_item_cvt: item %s: not enuf memory", tag);
	get_data_sub(istr, tag, type, bufin, dims, FALSE);
	    ...
	    put_data_sub(ostr, tag, type, bufin,  dims, FALSE); /* copy now */
            ...
            if (streq(cp,"f2d")) {		/* convert float to double */
                ...
                if (bufout == NULL)
               	    nerror("copy_item_cvt: item %s: (f2d) not enuf memory", tag);
...
void put_data(stream str, string tag, string typ, void *dat, int dim1, ...)
{
	...
	if (n >= MaxVecDim)			/*   no room for any more?  */
	    nerror("put_data: too many dims; item %s", tag);
    ...
    bool con)		/* coercion flag (not used) */
{
	...
	nerror("put_data_sub: putitem failed");
    freeitem(ipt, FALSE);			/* and reclaim storage      */
}

/************************************************************************/
...
void put_data_set(stream str, string tag, string typ, int dim1, ...)
{
    ...
    while (dim[n++] > 0) {			/* loop reading dimensions  */
	if (n >= MaxVecDim)			/*   no room for any more?  */
	    nerror("put_data_set: too many dims; item %s", tag);
	dim[n] = va_arg(ap, int);		/*   else get next argument */
    ...
    sspt = findstream(str);
    if (sspt->ss_ran)
        nerror("put_data_set: %s: can currently handle one random access item",tag);
...
void put_data_tes(stream str, string tag)
{
    ...
    sspt = findstream(str);
    ipt = sspt->ss_ran;
    if (ipt == NULL) nerror("put_data_tes: item %s is not random",tag);
    ipt = sspt->ss_ran;
    if (!streq(tag,ItemTag(ipt))) nerror("put_data_tes: invalid tag name %s",tag);
    ...
    int length)
{
    ...
    sspt = findstream(str);
    ipt = sspt->ss_ran;
    if (ipt==NULL) nerror("put_data_ran: tag %s no random item",tag);
    if (!streq(tag,ItemTag(ipt))) nerror("put_data_ran: invalid tag name %s",tag);
    offset *= ItemLen(ipt);     /* as input offset and length were  */
    length *= ItemLen(ipt);     /* in units of itemlen !!! */
    if (offset+length > datlen(ipt,0))
        nerror("put_data_ran: tag %s cannot write beyond allocated boundary",tag);
    fseeko(str,offset + ItemPos(ipt),0);
    if (length != fwrite((char *)dat,sizeof(byte),length,str))
        nerror("put_data_ran: error writing tag %s",tag);
}
    ...
    int length)
{
    ...
    sspt = findstream(str);
    ipt = sspt->ss_ran;
    if (ipt==NULL) nerror("put_data_blocked: tag %s no random item",tag);
    if (!streq(tag,ItemTag(ipt))) nerror("put_data_blocked: invalid tag name %s",tag);
    offset = ItemOff(ipt);
    length *= ItemLen(ipt);     /* in units of itemlen !!! */
    if (offset+length > datlen(ipt,0))
        nerror("put_data_blocked: tag %s cannot write beyond allocated boundary",tag);
    // no fseek() needed in blocked() !!!!
    // fseeko(str,offset + ItemPos(ipt),0);
    if (length != fwrite((char *)dat,sizeof(byte),length,str))
        nerror("put_data_blocked: error writing tag %s",tag);
    ItemOff(ipt) += length;
}
...
void get_set(stream str, string tag)
{
	...
	nerror("get_set: at EOF");
    if (! streq(ItemTyp(sptr), SetType))	/* be sure its a set        */
	nerror("get_set: %s not a set", tag);
    ss_push(sspt, sptr);
}
...
void get_data(stream str, string tag, string typ, void *dat, int dim1, ...)
{
    ...
    while (dim[n++] > 0) {			/* loop reading dimensions  */
	if (n >= MaxVecDim)			/*   no room for any more?  */
	    nerror("get_data: item %s: too many dims", tag);
	dim[n] = va_arg(ap, int);		/*   else get next argument */
    } 
    va_end(ap);
    						/* call next level routine  */
...
void get_data_coerced(stream str,string tag,string typ,void *dat,int dim1, ...)
{
    ...
    while (dim[n++] > 0) {			/* loop reading dimensions  */
	if (n >= MaxVecDim)			/*   no room for any more?  */
	    nerror("get_data_coerced: item %s: too many dims", tag);
	dim[n] = va_arg(ap, int);		/*   else get next argument */
    } 
    va_end(ap);
    						/* call next level routine  */
...
void get_data_set(stream str, string tag, string typ, int dim1, ...)
{
    ...
    sspt = findstream(str);
    if (sspt->ss_ran)
        nerror("put_data_set: %s: can only handle one random access item",tag);
    ipt = scantag(sspt,tag);	/* try and find the data */
    if (ipt==NULL) nerror("get_data_set: Bad EOF");
...
void get_data_tes(stream str, string tag)
{
    ...
    sspt = findstream(str);
    ipt = sspt->ss_ran;
    if (ipt == NULL) nerror("get_data_tes: item %s is not random",tag);
    ipt = sspt->ss_ran;
    if (!streq(tag,ItemTag(ipt))) nerror("get_data_tes: invalid tag name %s",tag);
...
void get_data_ran(
    ...
    ipt = sspt->ss_ran;
    if (ipt==NULL)
        nerror("get_data_ran: tag %s is not in random access mode",tag);
...
void get_data_blocked(
    ...
    offset = ItemOff(ipt);
    if (ipt==NULL)
        nerror("get_data_blocked: tag %s is not in blocked access mode",tag);
    copydata(dat,offset,length,ipt,str);
...
local bool puthdr(stream str, itemptr ipt)
{
    ...
    if (ItemDim(ipt) != NULL) {                 /* a vectorized item?       */
        if (xstrlen(ItemDim(ipt), sizeof(int)) > MaxVecDim)
            nerror("puthdr: too many dimensions");
...
local bool putdat(stream str, itemptr ipt)
{
	...
	nerror("putdat: item %s has no data", ItemTag(ipt));
...
local itemptr scantag(strstkptr sspt, string tag)
{
	...
	if (ipt != NULL && ! streq(tag, ItemTag(ipt)))
	    					/*   check tags match	  */
	    nerror("scantag: got %s instead of %s",
		  ItemTag(ipt), tag);
    } else {					/* input within a set?	  */
	ipt = finditem(sspt, tag);		/*   scan for the tag     */
	if (ipt == NULL)			/*   was none found?	  */
	    nerror("scantag: item %s not found in set %s",
...
local itemptr readitem(stream str, itemptr first)
{
	...
	if (bufp >= &ibuf[MaxSetLen])		/*   no room for next?	    */
	    nerror("readitem: set %s: buffer overflow", ItemTag(ip));
	np = getitem(str);		        /*   look at next item	    */
	if (np == NULL)				/*   at end of file?	    */
	    nerror("readitem: set %s: unexpected EOF", ItemTag(ip));
...
local itemptr gethdr(stream str)
{
	...
	if (typ == NULL)			/*   check for EOF          */
	    nerror("gethdr: EOF reading type");
        swap = FALSE;
    }
#if defined(CHKSWAP)
    else {        /* ISSWAP */
    ...
    stream str)
{
	...
	nerror("saferead: error calling fread %d*%d bytes", siz, cnt);
#if defined(CHKSWAP)
    ...
    int key)	      /* 0: start of file, 1: current point, 2: end */
{
	...
	nerror("safeseek: error calling fseeko %d bytes from %d",
	      offset, key);
}

/************************************************************************/
    ...
    int *dim)                 	/* dims, terminated with 0 */
{
	...
	nerror("makeitem: tag %s: malloc failed", tag);
...
local int baselen(string typ)
{
    ...
    nerror("baselen: type %s unknown", typ);	/* bad type string          */
...
local strstkptr findstream(stream str)
{
    ...
    if (stfree == NULL)				/* no free slot left?	    */
      nerror("findstream: no free slots, StrTabLen=%d",StrTabLen);
...
local void ss_push(strstkptr sspt, itemptr ipt)
{
	...
	nerror("get_set: Too many nested items");
...
local void ss_pop(strstkptr sspt)
{
	...
	nerror("ss_pop: stream stack underflow");
...
void strclose(stream str)
{
	...
	nerror("strclose: not at top level");
