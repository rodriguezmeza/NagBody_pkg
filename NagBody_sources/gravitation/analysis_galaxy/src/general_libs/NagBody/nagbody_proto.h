...
void PrintSnap(char *, char *, int, char *, char mode[2]);
void PrintBodies(string, string, int, int, int *, char *, char mode[2]);
void PrintBodiesSets(string, string, int, int, int *, int *, real, char *);

void SnapLessNBody(int *, int, real, char *, global_data_tree *,
		global_data_tree_bljforcecalc *);

bool readin_pl_file(string, int, int, int, int, int *);
bool readin_pl_file_3c(string, int, int, int, int, int, int *);
void readin_pl_snap(string, int, int, int *);
void readin_pl_snap_4c(string, int, int, int, int, int *);
void readin_pl_snap_type(string, int, int, int, int *);
...
// GRAVITACION 
void maketree_grav(bodyptr, int, global_data_treegrav *);
void normal_gravcalc(bodyptr, int, global_data_treegrav *);
void ind_normal_gravcalc(bodyptr, int, bodyptr, global_data_treegrav *);
//

// Several utilities proto definitions -----------------------------------------
void   AllocateMemory(short allocate_mode);			// To implement better i/o flexibility
void   allocate_memory(void);						// Will be substituted by the above
// Particle data structure to manipulate I/O
// N > 10^6 purpose...
void   allocate_memory_long(void);						// Will be substituted by the above
//
void   allocate_memory_double(void);				// Will be substituted by the above
void   free_memory(void);

void code_endrun(stream, real);
// -----------------------------------------------------------------------------
