...
string defv[] = {		";Invoke buildmap routine",
    ...
    "VERSION=1.3",		";Josh Barnes  12 May 2002",
    NULL,
};
	      ...
	      string prec, int ndim)
{
    ...
    sprintf(cmd, "%s %s %s -D%s -DNDIM=%d -o %s %s.c %s/lib/snapmap_%c%d.o "
            "-lNBody -lClib -lm", getenv("CC"), getenv("CCFLAGS"),
	    getenv("LDFLAGS"), prec, ndim, prog, prog, getenv("ZENOPATH"),
	    tolower(prec[0]), ndim);
