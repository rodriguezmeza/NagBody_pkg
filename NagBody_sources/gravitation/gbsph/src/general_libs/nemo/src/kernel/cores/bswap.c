...
void nemo_main(void)
{
	...
	nerror("Bad endian=%s; need 'big' or 'little' endian",endian);
    } else
      bptr = bswap;
    if (hasvalue("out"))
        outstr = nstropen(getparam("out"),"w");
    else {
 ...
 if (cnt < 0) {          /* streaming mode */
   ...
   strclose(outstr);
    ...
    if (cnt==0) nerror("File %s too small; nemo_file_size() returned 0",fname);
    data = (char *) allocate(cnt*len);
    t0 = 60 * cputime();
    if (cnt != fread(data,len,cnt,instr)) nerror("Error reading %s",fname);
    t1 = 60 * cputime();
    do {
      if (onetrip) {
    ...
    if (outstr) {
        if (cnt != fwrite(data,len,cnt,outstr)) 
            nerror("Error writing %s",getparam("out"));
        strclose(outstr);
    }
  }
}
