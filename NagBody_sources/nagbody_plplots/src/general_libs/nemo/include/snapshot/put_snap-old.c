...
//	    *mp++ = Mass(bp);;
	...
	free(mbuf);
#else
	nerror("put_snap_mass: NBMass undefined");
...
//#ifdef Phase
	...
	for (bp = *btptr, rvp = rvbuf; bp < *btptr + *nbptr; bp++) {
	    SETV(rvp, NBPhase(bp)[0]);
//	    SETV(rvp, Phase(bp)[0]);
	    rvp += NDIM;
	    SETV(rvp, NBPhase(bp)[1]);
//	    SETV(rvp, Phase(bp)[1]);
	    rvp += NDIM;
	}
	...
	free(rvbuf);
#else
	nerror("put_snap_phase: NBPhase undefined");
...
int *ofptr;			/* pointer to output bit flags */
{
	...
	free(pbuf);
#else
	nerror("put_snap_phi: Potential undefined");
#endif
    }
}
...
int *ofptr;			/* pointer to output bit flags */
{
	...
	free(abuf);
#else
	nerror("put_snap_acc: Acceleration undefined");
#endif
    }
}
...
int *ofptr;			/* pointer to output bit flags */
{
	...
	free(abuf);
#else
	nerror("put_snap_aux: Aux undefined");
#endif
    }
}
...
int *ofptr;			/* pointer to output bit flags */
{
	...
	free(kbuf);
#else
	nerror("put_snap_key: Key undefined");
#endif
    }
}
...
int *ofptr;			/* pointer to output bit flags */
{
	...
	free(abuf);
#else
	nerror("put_snap_dens: Dens undefined");
#endif
    }
}
...
int *ofptr;			/* pointer to output bit flags */
{
	...
	free(abuf);
#else
	nerror("put_snap_eps: Eps undefined");
#endif
    }
}
...
int *ofptr)			/* pointer to output bit flags */
{
	...
	nerror("put_snap_diagnostics: not implemented");
}
#else
