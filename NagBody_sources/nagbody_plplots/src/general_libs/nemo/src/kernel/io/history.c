...
{
    if (nhist > MAXHIST)
	nerror("ask_history: too much history");
...
void nemo_main()
{
    ...
    skip = getiparam("skip");
    if (skip<0) nerror("skip=%d cannot be negative",skip);

    if (hasvalue("in")) {
        instr = nstropen(iname, "r");                /* open input file */
    ...
    if (hasvalue("out")) {                       /* see if output needed */
        outstr = nstropen(oname, "w");               /*  open file */
