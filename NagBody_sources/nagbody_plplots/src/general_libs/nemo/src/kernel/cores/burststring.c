	...
	if (*lp == 0 || strchr(sep, *lp) != NULL) {	/*   is this a sep? */
		...
		*wp++ = (string) copxstr(strbuf, sizeof(char));
		if (wp == &wrdbuf[MWRD])		/*       no room? */
		    nerror("burststring: too many words (%d)",MWRD);
		sp = strbuf;				/*       for next 1 */
	    }
	} else {					/*   part of word */
	    *sp++ = *lp;				/*     so copy it */
	    if (sp == &strbuf[MSTR])			/*     no room? */
		nerror("burststring: word too long (%d)",MSTR);
	}
...
string *burst0string(string lst, string sep)
{
	...
	if (*lp == 0 || strchr(sep, *lp) != NULL) {	/*   is this a sep? */
		...
		*wp++ = (string) copxstr(strbuf, sizeof(char));
		if (wp == &wrdbuf[MWRD])		/*       no room? */
		    nerror("burststring: too many words\n");
		sp = strbuf;				/*       for next 1 */

	} else {					/*   part of word */
	    *sp++ = *lp;				/*     so copy it */
	    if (sp == &strbuf[MSTR])			/*     no room? */
		nerror("burststring: word too long\n");
	}
    } while (*lp++ != 0);				/* until list ends */
    *wp = NULL;
    return ((string *) copxstr((char *)wrdbuf, sizeof(string)));	/*PPAP*/
...
string *burst2string(string lst, string sep)
{
    ...
    do {						/* scan over list */
        ...
        if (news != olds) {
            ...
            *wp++ = (string) copxstr(strbuf, sizeof(char));
            if (wp == &wrdbuf[MWRD])		/*       no room? */
	        nerror("burst2string: too many words");
	    sp = strbuf;				/*       for next 1 */
	}
        *sp++ = *lp;				/*     so copy it */
	if (sp == &strbuf[MSTR])			/*     no room? */
	    nerror("burst2string: word too long");
        olds = news;
    } while (*lp++ != 0);				/* until list ends */
    *wp = NULL;
    return ((string *) copxstr((char *)wrdbuf, sizeof(string)));	/*PPAP*/
...
string sep)		/* chars which seperate words */
{
    ...
    nerror("splitstring: not implemented yet");
	...
	if (*lp == 0 || strchr(sep, *lp) != NULL) {	/*   is this a sep? */
		...
		*wp++ = (string) copxstr(strbuf, sizeof(char));
		if (wp == &wrdbuf[MWRD])		/*       no room? */
		    nerror("splitstring: too many words\n");
		sp = strbuf;				/*       for next 1 */
	    }
	} 
    } while (*lp++ != 0);				/* until list ends */
