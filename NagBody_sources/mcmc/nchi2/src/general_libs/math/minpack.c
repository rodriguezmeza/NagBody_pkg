...
// LevMarq method
// Mar :: funcs was extended (dparams) :: dextended!
...
// Mar :: added b funcs was extended (dparams)
//    double *dummyvec;
//    dummyvec=dvector(1,ma);
...
#define TINY 1.0e-10
//#define NMAX 5000 // Original line
#define NMAX 20000
#define GET_PSUM \
for (j=1;j<=ndim;j++) {\
...
//        if (*nfunk >= NMAX) nrerror("NMAX exceeded");     // Original line
        if (*nfunk >= NMAX) {
            fprintf(stdout,"\namoeba:: Warning!!! :: NMAX exceeded\n");
            return;
        }
        *nfunk += 2;
