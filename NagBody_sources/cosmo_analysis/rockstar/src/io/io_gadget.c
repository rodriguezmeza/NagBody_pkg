...
//  if (fabs(header->omega_0 + 0.046 + header->omega_lambda - 1.0) > 1e-2) {
    fprintf(stderr, "[Error] Mar Halo Finder Not Currently Configured to Run on Cosmologies with Curvature. (Omega_Matter = %f, Omega_Lambda = %f!)\n", header->omega_0, header->omega_lambda);
//    exit(1);
  }
