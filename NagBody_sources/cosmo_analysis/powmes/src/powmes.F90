...
!                              POWMES
!=======================================================================
! Authors : Stephane Colombi  
!           Dmitri Novikov 
!
! Publication : Colombi, Jaffe, Novikov, Pichon, 2009, MNRAS 393, 511
!
!-----------------------------------------------------------------------
!
! DISCLAIMER
...
!
! - Part of the calculations can be performed in simple precision, which
!   avoids copying arrays and saves significant amount of memory. The
!   compilation option -DSIMPLE can be used in the Makefile to do so.
!   Basically this option inhibits conversion of particle positions and
!   weights to double precision. This option has not been tested
!   sufficiently to be considered as "safe".
!
!-----------------------------------------------------------------------
!
! EXECUTION
...
!=======================================================================
  ...
  real(kind=8), allocatable, dimension(:,:) :: posD
  real(kind=8), allocatable, dimension(:) :: masspartD
...
#ifdef SIMPLE
     ! Use simple precision representation to save memory at the risk of a loss in accuracy
     ! Need to be tested further before becoming the default option
     call fold_particles3D(pos,npart,ifold,verbose)
     call powerspectrum3D(masspart,pos,npart,2*nyquist,pow(:,ifold), &
 &         powdebiased(:,ifold),nmod(:,ifold),staterr(:,ifold),norder,verbose,facto,eta)
     deallocate(pos,masspart)
#else
     ! Copy particle positions and weight in double precision arrays
     allocate(posD(3,0:npart-1),masspartD(0:npart-1))
     posD(1:3,0:npart-1)=pos(1:3,1:npart)
     masspartD(0:npart-1)=masspart(1:npart)
     deallocate(pos,masspart)
     
     call fold_particles3D(posD,npart,ifold,verbose)
     call powerspectrum3D(masspartD,posD,npart,2*nyquist,pow(:,ifold), &
 &         powdebiased(:,ifold),nmod(:,ifold),staterr(:,ifold),norder,verbose,facto,eta)
     deallocate(posD,masspartD)
#endif
     if (ifold==0) then
        do k=0,nyquist
...
!=======================================================================
  ...
  open(unit=lin,file=filein,form='unformatted',status='old',err=1,access='stream')
  read(lin) npart
  allocate(pos(3,npart))
  read(lin) ((pos(i,j),i=1,3),j=1,npart)
  if (read_mass) then
     allocate(masspart(npart))
     read(lin) (masspart(j),j=1,npart)
  endif
  close(lin)

  return

1 write(*,*) 'ERROR in read_binary : I cannot open the file'
  write(*,*) trim(filein)
  STOP
  ...
  if (filepower_fold(1:1) /= '#') then
     ...
     do k=0,ngrid/2
        do ifold=0,nfoldpow-1
           write(lunit,'(I12,$)') waven(k,ifold)
        enddo
        write(lunit,'(I12)') waven(k,nfoldpow)
     enddo
     close(lunit)
     ...
     do k=0,ngrid/2
        do ifold=0,nfoldpow-1
           write(lunit,'(E25.16,$)') nmod(k,ifold)
        enddo
        write(lunit,'(E25.16)') nmod(k,nfoldpow)
     enddo
     close(lunit)
     ...
     do k=0,ngrid/2
        do ifold=0,nfoldpow-1
           write(lunit,'(E25.16,$)') pow(k,ifold)
        enddo
        write(lunit,'(E25.16)') pow(k,nfoldpow)
     enddo
     close(lunit)  
     ...
     do k=0,ngrid/2
        do ifold=0,nfoldpow-1
           write(lunit,'(E25.16,$)') powdebiased(k,ifold)
        enddo
        write(lunit,'(E25.16)') powdebiased(k,nfoldpow)
     enddo
     close(lunit)  
     ...
     do k=0,ngrid/2
        do ifold=0,nfoldpow-1
           write(lunit,'(E25.16,$)') staterr(k,ifold)
        enddo
        write(lunit,'(E25.16)') staterr(k,nfoldpow)
     enddo
     close(lunit)     
...
!=======================================================================
subroutine fold_particles3D(phase_irreg,n_irreg,nfoldpower,verbose)
!=======================================================================
  use twopidef
  implicit none
  integer :: n_irreg,nfoldpower
#ifdef SIMPLE
  real(kind=4) :: phase_irreg(3,0:n_irreg-1)
#else
  real(kind=8) :: phase_irreg(3,0:n_irreg-1)
#endif
  logical :: verbose
  ...
  do i=0,n_irreg-1
     do j=1,3
        ...
        do while (myphase < 0.0d0)
           myphase=myphase+period
        enddo
        phase_irreg(j,i)=myphase
     enddo
  enddo
  phase_irreg(1:3,0:n_irreg-1)=phase_irreg(1:3,0:n_irreg-1)*2.0d0**nfoldpower
...
#ifdef SIMPLE
  real(kind=4) :: signal_irreg(0:n_irreg-1),phase_irreg(3,0:n_irreg-1)
#else
