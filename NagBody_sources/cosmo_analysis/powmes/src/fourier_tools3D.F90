...
!$OMP PRIVATE (i,ico,iphase,dphase,phaseh)
    do i=0,n_irreg-1
       do ico=1,3
          ...
          phase_irreg(ico,i)=dphase/norm(ico)
          do while (iphase < 0)
             iphase=iphase+n(ico)
          enddo
...
!$OMP PRIVATE (is,i,ii,ji,jj,jk)
 ...
 &                                     +(dble(phase_irreg(1,i))**iorder/factoi) &
 &                                     *(dble(phase_irreg(2,i))**jorder/factoj) &
 &                                     *(dble(phase_irreg(3,i))**korder/factok)*dble(signal_irreg(i))
#else                      
                      mysignal(ji,jj,jk)=mysignal(ji,jj,jk) &
 &                                     +(phase_irreg(1,i)**iorder/factoi) &
 &                                     *(phase_irreg(2,i)**jorder/factoj) &
 &                                     *(phase_irreg(3,i)**korder/factok)*signal_irreg(i)
#endif
                   enddo
                enddo
!$OMP END PARALLEL DO
...
!$OMP END PARALLEL DO
    ...
    deallocate(mysignal,ak,bk,iphi)
    do i=0,nx-1
