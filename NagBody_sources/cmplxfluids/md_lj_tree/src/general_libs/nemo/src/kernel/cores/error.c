...
#include "../../../src/tools/unproto/stdarg.h"
#include <errno.h>
...
{
    nerror("errorn is sadly not implemented; use debug>0 to get errno messages");
}

//void error(string fmt, ...)
void nerror(string fmt, ...)
{
    va_list_nemo ap;
...
nemo_main()
{
    ...
    n = getiparam("count");
    while(n-- > 0)
      nerror("error: foo=%f  bar=%d  fum=\"%s\"", 3.1415, 32768, "waldo");
}
