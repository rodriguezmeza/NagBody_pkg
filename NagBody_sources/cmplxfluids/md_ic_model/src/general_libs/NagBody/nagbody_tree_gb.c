/*==============================================================================
	MODULE: nagbody_tree_gb.c			[NagBody]
...
void maketree_t(bodyptr btab, int nbody, char *options, 
	global_data_tree *gdtree, global_data_tree_tljforcecalc *gdforce)
{
    double cpustart;
    bodyptr p;
    int i;
	...
	SETV( GPos(gdtree->root), Pos(gdtree->root) );		// Set GC ...
...
local void newtree(global_data_tree *gdtree)
{
...
local cellptr makecell(global_data_tree *gdtree)
{
    cellptr c;
    int i;
...
local void loadbody(bodyptr p, global_data_tree *gdtree)
{
    cellptr q, c;
    int qind, k;
    real qsize, dist2;
					...
					global_data_tree_tljforcecalc *gdforce)
{
    ...
    for (i = 0; i < NSUB; i++)
        if ((q = Subp(p)[i]) != NULL) {         
            ...
            Update(p) |= Update(q);
            Mass(p) += Mass(q);
			NBodies(p) += NBodies(q);		// Agregado para contener el num. de cuerpos
            MULVS(tmpv, Pos(q), Mass(q));
            ADDV(cmpos, cmpos, tmpv);
            MULVS(tmpv, Vel(q), Mass(q));	// Agregado para contener el num. de cuerpos
            ADDV(cmvel, cmvel, tmpv);		// Agregado para contener el num. de cuerpos
        }
    if (Mass(p) > 0.0) {                        
        DIVVS(cmpos, cmpos, Mass(p));
        DIVVS(cmvel, cmvel, Mass(p));		// Agregado para contener el num. de cuerpos
    } else {
        SETV(cmpos, Pos(p));
        SETV(cmvel, Vel(p));
    }
    ...
    SETV(Pos(p), cmpos);
    SETV(Vel(p), cmvel);					// Agregado para contener el num. de cuerpos
}
